...
### RAG智能搜索机制
RAG（Retrieval-Augmented Generation）搜索采用四阶段处理流程：

1. **标签分组筛选**：根据AI识别的服装类型进行分组筛选
   - 上装组：衬衫、T恤、毛衣、连衣裙
   - 下装组：裙子、裤子
   - 外套组：外套
   - 精确匹配 + 组内扩展（要求3个以上标签匹配）

2. **向量相似度计算**：使用CLIP模型提取的512维特征向量进行余弦相似度计算

3. **多因子综合评分**：结合向量相似度和标签相似度
   - 自适应权重机制：根据标签相似度动态调整权重
   - 协同效应奖励：当向量和标签相似度都较高时给予额外奖励

4. **阈值筛选**：支持用户自定义相似度阈值（0.0-1.0）

### 自适应权重算法
```
标签相似度 > 0.8：向量权重 0.4，标签权重 0.6
标签相似度 > 0.5：向量权重 0.5，标签权重 0.5
标签相似度 ≤ 0.5：向量权重 0.7，标签权重 0.3
```

### 标签权重优化
- **颜色权重降低**：从15%降至10%，减少颜色主导
- **服装类型权重提升**：提高到23-25%，强调款式重要性
- **类型特异性权重**：裙子/裤子使用专门权重配置，排除无关特征（领型、袖型）
